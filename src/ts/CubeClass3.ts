...
export class CubeWithPos {
    ...
    get colors() {
        return this.cubeConfig.colors;
    }

    get cubeStyle() {
        return this.cubeConfig.cubeStyle;
    }

    loadCube(cube: CubeWithPos) {
        this.cubeConfig.colors = cube.colors;
        this.cubeConfig.cubeStyle = cube.cubeStyle;
    ...
    reset() {
        SIDES.forEach((side: Side) => {
            ...
            for (let i = 0; i < Math.pow(this.dimensions, 2); i++) {
                ...
                const name =
                    this.pieceTypeStickerNames[pieceType][side][
                        stickerNameIndex
                    ];
                // console.log(name)
                const sticker = new Sticker(side, i, name);
    ...
    getStickersByType(types: PieceType[]) {
        return this.allStickers.filter((sticker) =>
            types.includes(sticker.type),
        );
    }
    ...
    getAllStickersInFilter(filter: (sticker: Sticker) => boolean) {
        const stickers = this.allStickers.filter(filter) as Sticker[];
        return stickers;
    }
    ...
    rotateColors(dimension: RotationDimension) {
    ...
    rotateColorsNTimes(dimension: RotationDimension, amount: number) {
